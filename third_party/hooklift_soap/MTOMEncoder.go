...
func newMtomEncoder(w io.Writer, attachments []MIMEMultipartAttachment) *mtomEncoder {
	return &mtomEncoder{
...
func (e *mtomEncoder) Encode(v interface{}) error {
	...
	for _, attachment := range e.attachments {
		...
		h.Set("Content-ID", fmt.Sprintf("<%s>", strings.Trim(attachment.Name, "<>")))
		h.Set("Content-Transfer-Encoding", "binary")
		if partWriter, err = e.writer.CreatePart(h); err != nil {
			return err
		}
		if _, err := partWriter.Write(attachment.Data); err != nil {
			return err
		}
	}
