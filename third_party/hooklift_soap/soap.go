...
const (
	...
	XmlNsSoapEnv12  string = "http://www.w3.org/2003/05/soap-envelope"
)
...
func (s *Client) call(ctx context.Context, soapAction string, request, response interface{}, faultDetail FaultError,
	...
	if s.opts.soapVersion == SOAP12 {
		envelope.XmlNS = XmlNsSoapEnv12
	}
	...
	if s.opts.mtom && s.opts.mma {
		...
		encoder = newMtomEncoder(buffer, s.attachments)
	} else if s.opts.mma {
	...
	if s.opts.mtom {
		req.Header.Set("Content-Type", fmt.Sprintf(mtomContentType, encoder.(*mtomEncoder).Boundary()))
	} else if s.opts.mma {
		req.Header.Set("Content-Type", fmt.Sprintf(mmaContentType, encoder.(*mmaEncoder).Boundary()))
	} else if s.opts.soapVersion == SOAP12 {
